...
void can_init(void)
{
    ...
    filter.FilterIdHigh = CAN_ID_TCU << 5;
    filter.FilterIdLow = CAN_ID_CCU << 5;

    if(DebugMode == CANDUMP)
    {
        filter.FilterMaskIdHigh = 0x8;
    } else {
        filter.FilterMaskIdHigh = ((~(CAN_ID_TCU ^ CAN_ID_SPEED)) << 5) | 0x8;
    }

    filter.FilterMaskIdLow = ((~(CAN_ID_CCU ^ CAN_ID_AVH_STATUS)) << 5) | 0x8;
